var fs = require("fs");
fs.readFile("SellStatistics.txt", function (err, data) {
    if (err) throw err;
    const input = data.toString();
    const record = input.split('\n');
    var collection = {};
    var myCal = function(result, collection, c, p_id, c_id, s_id, r_id){
        if (c_id == '_') {
        ...
        if (row[0] == 'S') {
            ...
            for (var c = d_start; c <= d_end; c++) {
                if(typeof collection[c] != 'undefined'){
                    if (p_id == -1) {
                        for (var idx_pid in collection[c]) {
                            result = myCal(result, collection, c, idx_pid, c_id, s_id, r_id);
                        }
                    } else {
                        if(typeof collection[c][p_id] != 'undefined'){
                            result = myCal(result, collection, c, p_id, c_id, s_id, r_id);
                        }
                    }
                }
            }
